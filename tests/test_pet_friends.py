...
def test_add_pets_with_valid_data(name = 'Keks', animal_type = 'Cat', age = '3', pet_photo = 'images/Keks.jpg'):
...
def test_delete_pet():
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pet(api_key, 'Keks', 'Cat', '3', 'images/Keks.jpg')
        _, my_pets = pf.get_list_of_pets(api_key, 'my_pets')
...
def test_update_pet_info(name = 'Sugar', animal_type = 'Kitty', age = '4'):
...
def test_add_pets_with_valid_data_without_photo(name = 'KeksWhitoutPhoto', animal_type = 'Cat', age = '3'):
    """Проверяем возможность добавления нового питомца без фото"""
    
    _, api_key = pf.get_api_key(valid_email, valid_password)
    status, result = pf.add_new_pets_without_photo(api_key, name, animal_type, age)
    
    assert status == 200
    assert result['name'] == name
    
def test_add_photo_at_pet(pet_photo = 'images/Sugar.jpg'):
    ...
    if len(my_pets['pets']) > 0:
        status, result = pf.add_photo_of_pets(api_key, my_pets['pets'][0]['id'], pet_photo)
...
def test_add_pets_negative_age_number(name = 'Keks', animal_type = 'Cat', age = '-8'):
    ...
    _, api_key = pf.get_api_key(valid_email, valid_password)
    _, result = pf.add_new_pets_without_photo(api_key, name, animal_type, age)
    
    assert age not in result['age'], 'Питомец добавлен на сайт с отрицательным числом в поле возраст'

def test_add_pets_with_four_digit_age_number(name = 'Keks', animal_type = 'Cat', age = '1234'):
...
def test_add_pet_with_a_lot_of_words_in_variable_name(animal_type='cat', age='2'):
...
def test_add_pet_with_special_characters_in_variable_animal_type(name='Sugar', age='3'):
...
def test_add_pet_with_numbers_in_variable_animal_type(name='Keks', animal_type='34562', age='3'):
...
def test_add_pet_with_a_lot_of_words_in_variable_animal_type(name='Sugar', age='2'):
